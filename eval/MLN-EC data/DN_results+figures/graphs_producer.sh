...
do
	...
	do
		scalatikz	-i ./${hle}/piec_lomrf_comparison_${measure}.csv -d $'\t' -P -c "blue" -m square* -x Thr -y "PIEC1-Micro"\
				-i ./${hle}/piec_lomrf_comparison_${measure}.csv -d $'\t' -P -c "red" -m square* -x Thr -y "LoMRF-Micro"\
				-t "${measure} for ${hle} LTA"\
				-X "Probability threshold"\
				-Y ${measure}\
				-a 0.0,1.0,0.0,1.0\
        	                -g "PIEC","MLN-EC (DN)"\
				-n dn_${hle}_${measure}_micro
